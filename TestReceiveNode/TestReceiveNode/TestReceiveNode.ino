...
void setupNRF24(){
  ...
  myRadio.startListening();    // Start listening for messages
...
void loop() {
  ...
  if(myRadio.available()){ //Check if recieved data
    Serial.println("Available");
    myRadio.read(&supsunder, sizeof(supsunder)); //read packet of data and store it in struct object
    if (supsunder.Sup != "")
    {
      Serial.print("SUP: ");
      Serial.println(supsunder.Sup); 
     
      lcd.clear();
      lcd.setCursor(2,0);
      lcd.print("SUP");
      lcd.setCursor(2,1);
      lcd.print(supsunder.Sup);
    }
  }
